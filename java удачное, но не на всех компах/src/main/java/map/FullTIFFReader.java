...
public class FullTIFFReader {
    ...
    public static void main(String[] args) {
        try {
            // Используем ресурс из JAR файла или внешний файл, если указан аргумент
            boolean success;
            ...
            if (args.length > 0) {
                ...
                System.out.println("Используем внешний файл: " + filePath);
                success = readTIFFFromFile(tiffFile);
            } else {
                // Иначе используем встроенный ресурс из JAR
                String resourcePath = "/out.tiff";
                java.io.InputStream inputStream = FullTIFFReader.class.getResourceAsStream(resourcePath);
                
                if (inputStream == null) {
                    System.out.println("Ресурс не найден: " + resourcePath);
                    System.out.println("Попробуйте указать путь к файлу как аргумент командной строки:");
                    System.out.println("java -jar height-profile-analyzer-1.0.0-executable.jar путь/к/файлу.tiff");
                    return;
                }
                
                System.out.println("Используем встроенный ресурс: " + resourcePath);
                success = readTIFFFromStream(inputStream);
            }
...
private static void saveArrayToCSV(String filename) {
    ...
    // Метод для чтения TIFF из файла
    private static boolean readTIFFFromFile(File tiffFile) throws Exception {
        Iterator<ImageReader> readers = ImageIO.getImageReadersByFormatName("TIFF");
    ...
    // Метод для чтения TIFF из InputStream (для ресурсов в JAR)
    private static boolean readTIFFFromStream(java.io.InputStream inputStream) throws Exception {
        Iterator<ImageReader> readers = ImageIO.getImageReadersByFormatName("TIFF");
        ...
        try (ImageInputStream stream = ImageIO.createImageInputStream(inputStream)) {
            ...
            System.out.println("Чтение потока данных...");
            System.out.println("Размеры: " + width + "x" + height);
